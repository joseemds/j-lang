...
void pp_unary_op(int op) {
    ...
    default:    printf("unknown_unary_op"); break;
    }
}
...
void pp_binary_op(int op) {
    ...
    default:     printf(" unknown_binary_op "); break;
    }
}
...
void pp_expr(ASTExpr *expr) {
    ...
    case EXPR_FUNC_CALL:
        ...
        printf(")");
        break;
    case EXPR_INT_LITERAL:      pp_int(expr->int_lit->value); break;
    case EXPR_FLOAT_LITERAL:    pp_float(expr->float_lit->value); break;
    case EXPR_STRING_LITERAL:   printf("\"%s\"", expr->string_lit->value); break;
    case EXPR_BOOL_LITERAL:     pp_bool(expr->bool_lit->value); break;
    case EXPR_CHAR_LITERAL:     printf("'%s'", expr->char_lit->value); break;
    case EXPR_IDENT:            pp_string(expr->ident->name); break;
    ...
    case EXPR_RATIONAL_LITERAL:
        printf("%d/%d", expr->rational_lit->value.top, expr->rational_lit->value.bot);
        break;
    default:
...
void pp_stmt(ASTStmt *stmt) {
    ...
    case STMT_FOR:
        printf("(for \n");
        printf("  (init "); pp_stmt(stmt->for_stmt->var); printf(")\n");
        printf("  (cond "); pp_expr(stmt->for_stmt->cond); printf(")\n");
        printf("  (inc "); pp_stmt(stmt->for_stmt->inc); printf(")\n");
        printf("  (body "); pp_stmt_list(stmt->for_stmt->body); printf(")\n");
        printf(")\n");
        break;
    ...
    case STMT_RETURN:   printf("(return "); pp_expr(stmt->return_stmt->expr); printf(")\n"); break;
    case STMT_BREAK:    printf("(break)\n"); break;
    case STMT_CONTINUE: printf("(continue)\n"); break;
    case STMT_EXPR:     printf("(expr_stmt "); pp_expr(stmt->expr->expr); printf(")\n"); break;
    default:            printf("(Unknown_Stmt_Kind)\n"); break;
    }
}

void pp_stmt_list(StmtList *stmt_list) {
    if (!stmt_list) printf(" (empty_block) ");
